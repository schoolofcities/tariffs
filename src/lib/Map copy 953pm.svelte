<script>
	...
	let map;
	let mapLoaded = false; // Add this flag

	let addressQuery = "";
	let addressResults = "";

	// Try absolute paths from root - Remove .gz extension
	...
	const selectTariffList = [
		"All goods subject to tariffs",
		"Automobiles",
		"Aluminum",
		"Steel",
		"Copper",
		"Lumber (before Oct 14)",
		"Lumber (after Oct 14)",
		"MHDV",
	...
	function updateMap() {
		if (
			...
			if (mapSelected) {
				const useOctData =
					dataLayers[mapSelected].tariffType ===
					"Lumber (before Oct 14)";

				if (mapQuery.metricType === "Percent") {
					// Show/hide appropriate polygon layers
					map.setLayoutProperty(
						"polygons",
						"visibility",
						useOctData ? "none" : "visible",
					);
					map.setLayoutProperty(
						"polygons_oct",
						"visibility",
						useOctData ? "visible" : "none",
					);
					...
					const activeLayer = useOctData
						? "polygons_oct"
						: "polygons";

					map.setPaintProperty(activeLayer, "fill-opacity", 0.8);
					...
					map.setPaintProperty(activeLayer, "fill-color", [
						...
						[
							...
							dataLayers[mapSelected].breaks[0],
							dataLayers[mapSelected].colours[1],
							dataLayers[mapSelected].breaks[1],
							dataLayers[mapSelected].colours[2],
							dataLayers[mapSelected].breaks[2],
							dataLayers[mapSelected].colours[3],
							dataLayers[mapSelected].breaks[3],
							dataLayers[mapSelected].colours[4],
						],
					]);
				} else if (mapQuery.metricType === "Count") {
					// Show/hide appropriate centroid layers
					map.setLayoutProperty("polygons", "visibility", "none");
					map.setLayoutProperty("polygons_oct", "visibility", "none");
					map.setLayoutProperty(
						"centroids",
						"visibility",
						useOctData ? "none" : "visible",
					...
					map.setPaintProperty(activeLayer, "circle-opacity", 0.5);
					map.setPaintProperty(activeLayer, "circle-stroke-width", 1);
					...
					map.setPaintProperty(activeLayer, "circle-color", [
						"case",
						[
							"==",
							["get", dataLayers[mapSelected].dataSource],
						...
						"rgba(0,0,0,0)",
						["==", ["get", dataLayers[mapSelected].dataSource], 0],
						"rgba(0,0,0,0)",
						[
							">",
							["get", dataLayers[mapSelected].dataSource],
					...
					map.setPaintProperty(activeLayer, "circle-stroke-color", [
						"case",
						[
							"==",
							["get", dataLayers[mapSelected].dataSource],
					...
					map.setPaintProperty(activeLayer, "circle-radius", [
						"interpolate",
						["linear"],
						["zoom"],
						3,
						[
							...
							dataLayers[mapSelected].size[4],
							[
								">",
								["get", dataLayers[mapSelected].dataSource],
								dataLayers[mapSelected].breaks[2],
							],
							dataLayers[mapSelected].size[3],
							0.5,
						],
						7.9999,
						[
						...
						[
							...
							dataLayers[mapSelected].size[2],
							[
								">",
								["get", dataLayers[mapSelected].dataSource],
								dataLayers[mapSelected].breaks[0],
							],
							dataLayers[mapSelected].size[1],
							dataLayers[mapSelected].size[0],
						],
					]);

					map.setLayoutProperty(activeLayer, "circle-sort-key", [
						"get",
						dataLayers[mapSelected].dataSource,
					]);
				} else {
					console.log("no data");
				}
			...
			console.log("map not loaded or style not ready");
		}
	}
	...
	onMount(async () => {
		...
		map.on("error", (e) => {
			...
			// If it's a decoding error, it's likely the PMTiles file issue
			if (e.error?.message?.includes("Decoding failed")) {
				console.error(
					"PMTiles decoding failed. Check if files are compressed or corrupted.",
				);
				console.error(
					"Ensure PMTiles files are in /public/pmtiles/ directory",
				);
				console.error(
					"Files should be .pmtiles format, not .pmtiles.gz",
				);
			}
		});
		...
		map.on("load", async () => {
			...
			// Add error handling for each source with more detailed logging
			try {
				console.log("Adding choropleth source from:", choropleth);
				map.addSource("choropleth", {
					type: "vector",
					url: "pmtiles://" + choropleth,
				});
				console.log("Choropleth source added");
			} catch (error) {
				console.error("Error adding choropleth source:", error);
			}

			try {
				console.log("Adding centroids source from:", centroids);
				map.addSource("centroids", {
					type: "vector",
					url: "pmtiles://" + centroids,
				});
				console.log("Centroids source added");
			} catch (error) {
				console.error("Error adding centroids source:", error);
			}

			try {
				console.log(
					"Adding censusDivisions source from:",
					censusDivisions,
				);
				...
				console.log("CensusDivisions source added");
			} catch (error) {
				console.error("Error adding censusDivisions source:", error);
			}

			try {
				console.log(
					"Adding choropleth_oct source from:",
					choropleth_oct,
				);
				...
				console.log("Choropleth_oct source added");
			} catch (error) {
				console.error("Error adding choropleth_oct source:", error);
			}

			// Add a separate centroids_oct source if you have one, otherwise it will use the same
			try {
				console.log(
					"Adding centroids_oct source (same as choropleth_oct)",
				);
				map.addSource("centroids_oct", {
					type: "vector",
					url: "pmtiles://" + choropleth_oct,
				});
				console.log("Centroids_oct source added");
			} catch (error) {
				console.error("Error adding centroids_oct source:", error);
			}
			...
			try {
				...
				console.log("Polygons layer added successfully");
			} catch (error) {
				console.error("Error adding polygons layer:", error);
			}
			...
			try {
				...
				console.log("Polygons_oct layer added successfully");
			} catch (error) {
				console.error("Error adding polygons_oct layer:", error);
			}
			...
			map.once("idle", () => {
				mapLoaded = true; // Set flag after map is fully loaded
				updateMap();
			});
		});
		...
		map.on("style.load", () => {
			...
			// Set loaded flag when style is ready
			if (!mapLoaded) {
				setTimeout(() => {
					mapLoaded = true;
					updateMap();
		...
		map.on("mousemove", "polygons_oct", (e) => {
			const now = performance.now();
			if (now - lastUpdate < 100) return;
			lastUpdate = now;
			...
			if (currentZone !== selectedZone) {
				...
				map.setFilter("outline-hover_oct", [
					"==",
					"ADADGUID",
					selectedZone,
				]);
		...
		map.on("mouseleave", "polygons_oct", () => {
			map.getCanvas().style.cursor = "";
			selectedZone = "";
			selectedValue = "";
			map.setFilter("outline-hover_oct", ["==", "ADADGUID", ""]);
		});
		...
		map.on("mousemove", "centroids_oct", (e) => {
			const now = performance.now();
			if (now - lastUpdate < 100) return;
			lastUpdate = now;
		...
		map.on("mouseleave", "centroids_oct", () => {
			map.getCanvas().style.cursor = "";
			selectedZone = "";
			selectedValue = "";
			map.setFilter("outline-hover_oct", ["==", "ADADGUID", ""]);
	...
	const getResults = async () => {
		let inputQuery =
			addressQuery.endsWith("Canada") ||
			addressQuery.endsWith("CA") ||
			addressQuery.endsWith("Can")
				? addressQuery
				: addressQuery + ", Canada";

		addressResults = await fetch(NOMINATIM_URL + inputQuery).then((res) =>
			res.json(),
		);
	...
	<div id="panel">
		<div class="logo-container">
			...
			>
				<img
					src={logoBlueColour}
					alt="UofT and School of Cities logos"
					class="logo"
				/>
			</a>

			<a href="./" target="_blank" class="research-link"
				>Mapping Tariffs</a
			>
		</div>
		...
		<p style="font-size: 14px; margin-top: 25px; line-height: 20px;">
			By <a
				href="https://mkbs-mkbs2000.github.io/Personal-Portfolio/"
				target="_blank">Muhammad Khalis Bin Samion</a
			>,
			<a href="https://jamaps.github.io/" target="_blank"> Jeff Allen</a>,
			<a
				href="https://discover.research.utoronto.ca/8035-tara-vinodrai"
				target="_blank">Tara Vinodrai</a
			>,
			<a
		...
		<div id="select-wrapper">
			<div id="destext">
				<p style="margin-bottom: -5px;">
					Select the good/product subject to U.S. tariffs:
				</p>
			</div>
			<Select
				id="select"
		...
		<div id="destext">
			<p style="margin-bottom: -5px;">Select an indicator:</p>
		</div>
		<div class="button-group" style="margin-top: 10px;">
			<div
				class="toggle-button {impactType === 'Business'
					? 'selected'
					: ''}"
			...
			<div
				class="toggle-button {impactType === 'EmployeeWork'
					? 'selected'
					: ''}"
			...
			<div
				class="toggle-button {impactType === 'EmployeeHome'
					? 'selected'
					: ''}"
		...
		<div class="button-group">
			<div
				class="toggle-button {metricType === 'Percent'
					? 'selected'
					: ''}"
				...
				<div id="legend">
					<svg width="500" height="40">
						...
						<text
							...
							y="35"
							>&lt;{(
								dataLayers[mapSelected].breaks[0] * 100
							).toFixed(0)}%</text
						>
						<text
							class="legend-label"
							...
							x="130"
							y="35"
							>{(dataLayers[mapSelected].breaks[1] * 100).toFixed(
								0,
							)}%</text
						>
						<text
							...
							x="195"
							y="35"
							>{(dataLayers[mapSelected].breaks[2] * 100).toFixed(
								0,
							)}%</text
						>
						<text
							...
							y="35"
							>&gt{(
								dataLayers[mapSelected].breaks[3] * 100
							).toFixed(0)}%</text
						>
			...
			{:else if dataLayers[mapSelected]?.metricType === "Count" && dataLayers[mapSelected]?.colours}
				...
				<div id="legend">
					<svg width="350" height="200">
						...
						<text class="legend-label" x="100" y="35" dy="0.35em"
							>&gt;{dataLayers[mapSelected].breaks[3]}</text
						>
						<text class="legend-label" x="100" y="96" dy="0.35em"
							>{dataLayers[mapSelected].breaks[2] + 1} - {dataLayers[
								mapSelected
							].breaks[3]}</text
						>
						<text class="legend-label" x="100" y="137" dy="0.35em"
							>{dataLayers[mapSelected].breaks[1] + 1} - {dataLayers[
								mapSelected
							].breaks[2]}</text
						>
						<text class="legend-label" x="100" y="160" dy="0.35em"
							>{dataLayers[mapSelected].breaks[0] + 1} - {dataLayers[
								mapSelected
							].breaks[1]}</text
						>
						{#if dataLayers[mapSelected].breaks[0] !== 0}<text
								class="legend-label"
								x="100"
								y="182"
								dy="0.35em"
								>&le;{dataLayers[mapSelected].breaks[0]}</text
							>{/if}
					</svg>
				</div>
			{/if}
		...
		<div id="hovered-zone">
			<i>Hovered zone</i>: {@html selectedValue
		...
		<div class="datadetail">
			<p>
				Counts of Employment (home) data are based on estimates from the
				2021 Census of Population. Counts of Businesses and Employment
				(place of work) and are based on estimates from the Canadian
				Business Register (December 2022).
			</p>

			<h4 style="margin-bottom: 0px;">Data sources list:</h4>
			<p>
				Layers on this map were created by combining data from the
				following sources:
			</p>
			<ul>
				...
				<li>
					...
					(Statistics Canada Catalogue No. 71-607-X2021004)
				</li>
				<li>
					Cargo Systems Messaging Service (United States Customs and
					Border Protection)
				</li>
				<li>
					Harmonized Tariff Schedule of the United States (United
					States International Trade Commission )
				</li>
				<li>
					International Trade and Development Division (Statistics
					Canada)
				</li>
			...
			<p>
				For detailed data descriptions, download links, and processing
				steps, please read our <a
					href="https://github.com/schoolofcities/tariffs?tab=readme-ov-file"
					target="_blank">data and methodology page</a
				>
			</p>
