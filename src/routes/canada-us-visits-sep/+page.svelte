...
<script>
	...
	import { mean, sum, median, max as d3Max, min as d3Min } from 'd3-array';
	...
	let selection = {
		...
		year2: 2025,
		year3: 2026,
		period1Start: "2024-08-01",
		period1End: "2025-07-31",
		period2Start: "2025-08-01",
		period2End: "2026-07-31",
	...
	function isPortlandOR(metroName) {
		const normalized = metroName.replace(' Micro Area', '').trim();
		const cityPart = normalized.split(',')[0].trim();
		const shortCity = cityPart.split(/[-/]/)[0].trim();
		const stateMatch = normalized.match(/,\s*([A-Z]{2}(?:-[A-Z]{2})*)/);
		const states = stateMatch ? stateMatch[1].split('-') : [];
		return shortCity === 'Portland' && states.includes('OR');
	}

	// Reusable YoY percent-change calculator, independent of the `selection`
	// period used for the main display, so we can compare different period
	// definitions (e.g. Apr-Mar vs Aug-Jul) without touching the UI state.
	function computeYoYPercentChanges(data, metroList, p1StartStr, p1EndStr, p2StartStr, p2EndStr) {
		...
		metroList.forEach(metro => {
			const metroData = data.filter(d => d.metro === metro);
			const period1Data = metroData.filter(d => d.date >= p1Start && d.date <= p1End);
			const period2Data = metroData.filter(d => d.date >= p2Start && d.date <= p2End);
			if (period1Data.length < 10 || period2Data.length < 10) return;

			const avg1 = mean(period1Data, d => d.normalized);
			const avg2 = mean(period2Data, d => d.normalized);
			if (!(avg1 > 0)) return;

			changes.push(((avg2 - avg1) / avg1) * 100);
		});
		return changes;
	}
	...
	function processData(normalizedDataRaw) {
		...
		normalizedDataRaw.forEach(row => {
			...
			const normalizedValue = row.TRIPS_PER_ALL_DEVICES ?? row.NORMALIZED;
		...
		// Filter to our date range. Widened to April 1, 2024 - July 31, 2026 so
		// it covers both the Apr-Mar and Aug-Jul YoY period definitions.
		const startDate = new Date("2024-04-01");
		const endDate = new Date("2026-07-31");
		
		processedData = normalizedData.filter(
			d => d.date >= startDate && d.date <= endDate && !isPortlandOR(d.metro)
		);
	...
	onMount(() => {
		...
		// Log the YoY median % change across all metros for two different
	// period definitions, independent of whichever one `selection` uses
	// for the on-page display.
	$: if (dataLoaded && processedData.length > 0) {
		const aprMarChanges = computeYoYPercentChanges(
			processedData,
			metros,
			"2024-04-01",
			"2025-03-31",
		...
		const augJulChanges = computeYoYPercentChanges(
			processedData,
			metros,
			"2024-08-01",
			"2025-07-31",
		...
		console.log(
			"YoY median % change, Apr 1 2024 - Mar 31 2026 (Apr-Mar split):",
			median(aprMarChanges),
